...
- 客户端宽度 < 100 列（例如手机 Termius SSH）：**手机模式**
  ...
  - 在根键表吞掉 `Up` / `Down`：手机终端常把手指上下滑动合成成方向键发给远端，被吞掉后就不会扰乱 shell 历史或 vim 光标（copy-mode 另有独立键表，真实方向键仍可用）。`PageUp` 改成直接进 copy-mode 向上翻页，给"想看回滚历史"留一个显式入口。想让手机端真正用 Termius 本地滚动，还需要在 Termius 设置里把滑动手势由"Arrow keys"改为"Local scroll"或"Disabled"。
